...
while(True):
    ...
    if keycode.get(p.B3G_RETURN) == 1:        #If ENTER key is pressed then the simulation executes
        ...
        while(True):
            ...
            print("The height is {}".format(state),action,integral)
            ...
            if keycode.get(p.B3G_RETURN) == 1 or (ts/240) == 30:    #Reseting the simulation when Enter is pressed
                ...
                gph.plot(ax,ay)
                gph.hlines([desired_state], 0, 1, transform=gph.get_yaxis_transform(), colors='r')
                gph.grid()
                gph.set_title('height vs time')
                gph.set_xlabel('time (s)')
                gph.set_ylabel('amplitude')

                fig.suptitle('PID CONTROL', fontsize=16)
